...
impl Iface {
    ...
    async fn add(&self, payload: Vec<u8>, mime: &str) -> zbus::fdo::Result<u64> {
        ...
        tokio::task::spawn_blocking(move || -> zbus::fdo::Result<u64> {
            ...
            let server = open_server()?;
            // `payload_memfd` always hands us a memfd, which statx reports as a
            // regular file, so the unchecked variant is safe here. This mirrors
            // what the x11 and wayland watchers do.
            let resp =
